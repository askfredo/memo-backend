...
class PasswordVaultController {
    ...
    async createPassword(req, res) {
        try {
            const { title, username, email, password, url, notes, category, icon, userId = '00000000-0000-0000-0000-000000000001' } = req.body;
            if (!title || !password) {
                return res.status(400).json({ error: 'Title and password are required' });
            }
            const encryptedPassword = this.encrypt(password);
            const result = await index_1.db.query(`INSERT INTO password_vault 
         (user_id, title, username, email, password_encrypted, url, notes, category, icon)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         RETURNING id, title, username, email, url, notes, category, icon, is_favorite, created_at, updated_at`, [userId, title, username, email, encryptedPassword, url, notes, category || 'general', icon || '🔐']);
    ...
    async getPassword(req, res) {
        try {
            ...
            const result = await index_1.db.query(`SELECT * FROM password_vault WHERE id = $1 AND user_id = $2`, [passwordId, userId]);
    ...
    async updatePassword(req, res) {
        try {
            ...
            const result = await index_1.db.query(`UPDATE password_vault 
         SET ${updates.join(', ')}
         WHERE id = $${paramCount} AND user_id = $${paramCount + 1}
         RETURNING id, title, username, email, url, notes, category, icon, is_favorite, created_at, updated_at`, values);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Password not found' });
            }
            res.json({ password: result.rows[0] });
    ...
    async deletePassword(req, res) {
        try {
            const { passwordId } = req.params;
            const userId = '00000000-0000-0000-0000-000000000001';
            const result = await index_1.db.query('DELETE FROM password_vault WHERE id = $1 AND user_id = $2 RETURNING *', [passwordId, userId]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Password not found' });
            }
...
exports.passwordVaultController = new PasswordVaultController();
